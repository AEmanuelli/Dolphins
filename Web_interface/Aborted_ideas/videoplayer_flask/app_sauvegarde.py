...
import subprocess
@app.route('/php/<path:path>')
def serve_php(path):
    php_file_path = os.path.join('php', path)
    php_output = subprocess.check_output(['php', php_file_path])
    return Response(php_output, content_type='text/html')
